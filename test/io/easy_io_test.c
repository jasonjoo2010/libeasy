...
TEST(easy_io, wait)
{
    ...
    easy_io_add_listen(eio, NULL, 9998, &io_handler);
    easy_io_add_listen(eio, NULL, 9999, &io_handler);
...
static void *test_uthread_start(void *args)
{
    test_iocp_t             *cp = (test_iocp_t *) args;
...
static void test_simple_fork()
{
    ...
    for(i = 0; i < cnt; i++) {
        ...
        for(j = 0; j < 10; j++, port ++) {
            if ((listen = easy_io_add_listen(eio, NULL, port, &io_handler)))
                break;
        }
